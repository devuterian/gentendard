...
## Local Divergence (Gentendard)

- **Project id:** `gentendard` (GitHub: `devuterian/gentendard`).
- **Artifacts:** Font binaries and release zips are built locally (`make build`, `make package`); `dist/` and `build-release/` are gitignored. Upstream font trees under `vendor/` after `fetch_vendor.sh` are gitignored; only `vendor/README.md` is tracked.
- **Build tooling:** OTF export uses FontForge CLI in addition to Python `fonttools` (see `docs/build.md`).
- **Upstream pins:** Documented in `vendor/README.md` and `records/research/RSH-20260506-001.md`.
